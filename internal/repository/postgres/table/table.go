...
func (r Repository) AdminGetList(ctx context.Context, filter tables.Filter) ([]tables.AdminGetList, int, error) {
	...
	table := "tables"
	whereQuery := fmt.Sprintf(`WHERE %s.deleted_at IS NULL and branches.restaurant_id='%d'`, table, *claims.RestaurantID)
...
func (r Repository) AdminUpdateAll(ctx context.Context, request tables.AdminUpdateRequest) error {
	...
	if err = r.ValidateStruct(&request, "ID", "Number", "Status", "Capacity", "BranchID"); err != nil {
		return err
	}
